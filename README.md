...
The main purpose of this reproduction study is to test evaluate Hoffman's claim that kPCA is a superior anomaly detection method to the baseline comparisons he presents. This evaluation differed from Hoffman's original work by using a hold out validation set for parameter tuning in addition to incorporating 4 new anomaly datsets for evaluation.

An interesting result is that the three different kernel-based methods (kPCA, ParzenWindow, and OC-SVM) have similar regions of detection for similar values of sigma given a particular data set. For OC-SVM and kPCA, the choice of sigma dominates performance over the other two parameter choices (nu and q respectively), however, there are choices that can significantly improve detection, as seen in the 'island' of higher AUC for kPCA in the Glass Validation.  

kPCA had the higher AUC across a number of datasets as Hoffman reports, but the split on the validation set shows one issue of generalization. Again, Hoffman did not use a validation set, but showed the effect of parameter settings directly on the test set. In this reproduction study, the paramter setting for kPCA that were best during validation, were not necessarily the best during testing, as seen in the 'Glass' and, to a lesser extent, the 'Ionosphere' datasets . Additionally, a finer grid search than that shown in [[1](https://www.sciencedirect.com/science/article/pii/S0031320306003414)] allowed for a more competitive comparison between OC-SVM, kPCA, and the Parzen Window. On several datasets, each of these performed nearly perfectly, with corresponding AUCs very close to or equalling 1.0. Different splits of validation and testing change the comparison for these very close results, this can be seen by varying the random seed which will change the randomized split.

Hoffman also produced a boundary comparison in his evaluation, however he used noisy data to train the fit the models. I supsect this is simply because it is more straightforward to impose noise untop of of background points, rather than preventing the overlap. In this evaluation, the results on the two toy examples 'Circles' and 'Roll' use 'clean' data to train the models. Following these methods the differences between kPCA and OC-SVM are less dramatic. However, there is still a clear advantage over the Parzen Window which shows a lack of generalization, replicating Hoffman's findings. 

Overall, I was able to reproduce the results of Hoffman's paper and demonstrate the benfit of kPCA as a novelty detection algorithm. However, the inclusion of a validation set for paramter tuning shows some potential issues for selecting the correct values of sigma and q when applying kPCA. 

### Dependencies and Code
All tests were run in python 3.6 in Windows 10 using an Intel i5-82500CPU @1.6GHz with 20GB RAM
